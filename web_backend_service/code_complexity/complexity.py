...
class Checker:
    def __init__(self, file_path: str):
        ...
        for i in range(len(self)):
            self.indentation_structure.append(len(self.codes[i]) - len(self.codes[i].lstrip()))
    ...
    def cal_main_complexity(self, main_codes: list) -> CompStr:
        ...
        :return:
        """
        if not len(main_codes):
            return CompStr('')
        main_indentation = []
        for i in range(len(main_codes)):
            main_indentation.append(len(main_codes[i]) - len(main_codes[i].lstrip()))
    ...
    def integrate_complexity(begin: int, end: int, indentation_structure: list, complexity_tag: list) -> CompStr:
        ...
        return CompStr("1") if not len(comp_record) else max(comp_record)
